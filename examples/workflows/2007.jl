...
mdl2007 = @model begin
    @param begin
        ...
        TVVC ∈ RealDomain(lower=0.0001, upper=100.0, init=30.0)
        TVKA ∈ RealDomain(lower=0.0001, upper=5.0, init=0.1)
        TVQ ∈ RealDomain(lower=0.0001, upper=50.0, init=4.0)
        TVVP ∈ RealDomain(lower=0.0001, upper=1000.0, init=100.0)
        EXP_WT ∈ RealDomain(lower=0.0001, upper=3.0, init=0.75)
        COV_SEX ∈ RealDomain(lower=0.0001, upper=2.0, init=1.0)
...
param = init_param(mdl2007)

fit_2007=nothing
@time fit_2007 = fit(mdl2007, data, param, Pumas.FOCEI())
@time inf_2007 = infer(fit_2007)
