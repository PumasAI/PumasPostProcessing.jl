...
mdl2004 = @model begin
    @param begin
        ...
        #EXP_WT ∈ RealDomain(lower=0.0001, upper=1000.0, init=0.75)
    ...
    @pre begin
        CL = TVCL * exp(η[1]) * (WEIGHT/70)^0.75 #* (COV_SEX^SEX)
        VC = TVVC * exp(η[2])
        KA = TVKA * exp(η[3])
        Q = TVQ
        VP = TVVP
...
param = init_param(mdl2004)

fit_2004=nothing
@time fit_2004 = fit(mdl2004, data, param, Pumas.FOCEI())
@time inf_2004 = infer(fit_2004)
